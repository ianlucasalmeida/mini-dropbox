...
def index(path):
    ...
    folders = File.query.filter(and_(File.user_id == current_user.id, File.path == current_path, File.is_folder == True)).order_by(File.name).all()
    files_db = File.query.filter(and_(File.user_id == current_user.id, File.path == current_path, File.is_folder == False)).order_by(File.name).all()
...
def upload_file():
    ...
    if File.query.filter_by(user_id=current_user.id, path=current_path, name=filename).first():
        flash(f'Um item chamado "{filename}" já existe nesta pasta.', 'danger')
        return redirect(current_path)
    path_parts = [part for part in current_path.split('/') if part]
    path_parts.append(filename)
    s3_path = '/'.join(path_parts)
    s3_key = f"user_{current_user.id}/{s3_path}"
...
def create_folder():
    ...
    if not folder_name:
        flash('O nome da pasta não pode ser vazio.', 'warning')
        return redirect(current_path)
    if File.query.filter_by(user_id=current_user.id, path=current_path, name=folder_name).first():
        flash(f'Uma pasta ou arquivo chamado "{folder_name}" já existe.', 'danger')
        return redirect(current_path)
...
def delete_item(item_id):
    ...
    if not item or item.user_id != current_user.id:
        flash("Item não encontrado ou acesso negado.", "danger")
        return redirect(url_for('index'))
    ...
    if not item.is_folder:
        try:
            ...
            flash(f"Erro ao apagar o arquivo: {e}", "danger")
            db.session.rollback()
    else:
        try:
            ...
            db_records_to_delete_q = File.query.filter(File.user_id == current_user.id, or_(File.id == item_id, File.path == folder_full_path, File.path.startswith(folder_full_path + '/')))
            db_records_to_delete_q.delete(synchronize_session=False)
            db.session.commit()
            flash(f'Pasta "{item.name}" e todo o seu conteúdo foram apagados.', 'success')
        except Exception as e:
            flash(f"Erro ao apagar a pasta: {e}", "danger")
            db.session.rollback()
...
def download_file(file_id):
    ...
    try:
        ...
        return redirect(file_to_download.path)
...
def download_folder(folder_id):
    ...
    if not folder or folder.user_id != current_user.id or not folder.is_folder:
        flash("Acesso negado ou item inválido.", "danger")
        return redirect(url_for('index'))
    ...
    with zipfile.ZipFile(memory_file, 'w', zipfile.ZIP_DEFLATED) as zip_archive:
        ...
        if not files_to_zip:
            flash("A pasta está vazia, nada para baixar.", "warning")
        ...
        for file_db in files_to_zip:
            try:
                ...
                print(f"Não foi possível adicionar o arquivo {file_db.s3_key} ao zip: {e}")
    memory_file.seek(0)
...
def tag_file():
    ...
    file_to_tag.tag = tag if tag != 'none' else None
    db.session.commit()
    return jsonify({'success': True, 'message': 'Tag atualizada com sucesso!'})

# --- NOVAS ROTAS PARA MOVER ARQUIVOS ---
...
def get_folder_tree():
    ...
    folder_tree = {'/': {'name': 'Raiz', 'subfolders': {}}}
    ...
    for folder in folders:
        ...
        for part in path_parts:
            if part not in current_level:
                current_level[part] = {'name': part, 'subfolders': {}}
...
def move_item_recursive(item_id, new_path, s3_client, db):
    item = db.session.get(File, item_id)
    if not item or item.user_id != current_user.id:
        raise Exception("Item não encontrado ou acesso negado.")
        
    old_full_path = os.path.join(item.path, item.name).replace('//', '/')

    if not item.is_folder:
        # Move um arquivo
        new_s3_key_path = os.path.join(new_path, item.name).strip('/')
        new_s3_key = f"user_{current_user.id}/{new_s3_key_path}"
        
        # Copia e apaga no S3
        copy_source = {'Bucket': S3_BUCKET_NAME, 'Key': item.s3_key}
        s3_client.copy_object(CopySource=copy_source, Bucket=S3_BUCKET_NAME, Key=new_s3_key)
        ...
        # Move uma pasta e seu conteúdo
        # Encontra todos os itens dentro da pasta
        children_items = File.query.filter(
            File.user_id == current_user.id,
            File.path.startswith(old_full_path)
        ).all()

        for child in children_items:
            # Calcula o novo caminho para o filho
            child_relative_path = os.path.relpath(child.path, old_full_path)
            child_new_path = os.path.join(new_path, item.name, child_relative_path).replace('//', '/')
            move_item_recursive(child.id, child_new_path, s3_client, db)
...
def move_item(item_id):
    ...
    try:
        move_item_recursive(item_to_move.id, target_path, s3_client, db)
        db.session.commit()
    ...
    return redirect(item_to_move.path)
